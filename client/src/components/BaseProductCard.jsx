...
export const BaseProductCard = ({
  ...
  hasExpired = false, 
  hideStockInfo = false,
  displayStock, // ✨ FIX: Accept a custom stock override
  children
}) => {
  // ✨ FIX: Use the override if provided, otherwise fallback to db totalStock
  const totalStock = displayStock !== undefined ? displayStock : (product.totalStock || 0);
  ...
  return (
    <div className={`bg-white rounded-2xl border ${hasExpired ? 'border-red-300' : 'border-slate-200'} overflow-hidden mb-3 shadow-sm transition-all`}>
      <button onClick={onToggle} className="relative w-full text-left hover:bg-slate-50 transition-colors duration-200">
        <div className="flex items-center gap-3 pr-6">
          ...
          <div className="shrink-0 m-1 w-24 h-24 relative rounded-xl overflow-hidden bg-slate-100 border border-slate-200 shadow-sm z-10">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-black/80 px-1 py-0.5 pointer-events-none">
              <p className="text-[9px] font-black uppercase tracking-widest text-white text-center truncate" title={companyDisplay}>
                {companyDisplay}
              </p>
            </div>
          </div>
        ...
        {!hideStockInfo && (
          <div className="absolute -bottom-px -right-px flex items-stretch overflow-hidden rounded-tl-xl shadow-sm border-t border-l border-slate-200 bg-white">
            {hasExpired ? (
              <span className="text-xs font-black px-2.5 py-1 bg-red-600 text-white flex items-center border-r border-red-700">
                EXPIRED BATCH
              </span>
            ) : hasNearExpiry ? (
            ...
            <div className="px-3 py-1 flex items-baseline gap-1.5 bg-slate-50">
              <span className={`font-black text-lg leading-none ${stockStatus === 'out' ? 'text-red-500' : stockStatus === 'low' ? 'text-amber-500' : 'text-emerald-600'}`}>
                {totalStock} {/* ✨ FIX: Renders the dynamic prop! */}
              </span>
