...
export const TopProducts = ({ period = 'month' }) => (
  <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
      ...
      {topProducts.map(({ name, company, sold, revenue }, i) => (
        <div key={name} className="px-4 py-3.5 flex items-center gap-3">
          ...
          <div className="text-right shrink-0">
            <p className="text-slate-800 text-sm font-bold">{revenue}</p>
            <p className="text-slate-400 text-xs">{sold.toLocaleString()} units</p>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="px-4 py-3 border-t border-slate-100">
      <Link to="/admin-dashboard/products" className="text-emerald-600 text-sm font-semibold flex items-center gap-1">
