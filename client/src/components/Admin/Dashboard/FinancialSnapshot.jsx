// components/Admin/Dashboard/FinancialSnapshot.jsx
import { Link } from 'react-router-dom';
import { ArrowUpRight, ArrowRight } from 'lucide-react';
import { financialData } from './constants';

export const FinancialSnapshot = ({ period, setPeriod }) => (
  <div className="bg-slate-900 rounded-2xl overflow-hidden">
        ...
        {['month', 'year'].map(p => (
          <button
            ...
            className={`px-3 py-1 rounded-lg text-xs font-bold transition-all capitalize
              ${period === p ? 'bg-emerald-500 text-white shadow' : 'text-slate-400 hover:text-white'}`}
          >
    ...
    <div className="divide-y divide-slate-700/30">
      {[financialData[period].sales, financialData[period].collection, financialData[period].outstanding].map(({ value, label, sub, positive }) => (
