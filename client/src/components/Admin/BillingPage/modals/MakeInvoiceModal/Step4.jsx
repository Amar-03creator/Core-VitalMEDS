// Step4.jsx
import { CheckCircle2, Download, PlusCircle, FileText, Edit3 } from 'lucide-react';
import { Link } from 'react-router-dom';

export const Step4 = ({ generatedInvoice, onDownloadPDF, onNewInvoice, onEditInvoice, onClose }) => (
  <div className="space-y-5">
    ...
    <div className="flex flex-col gap-3">
      <button onClick={onDownloadPDF} className="w-full flex items-center justify-center gap-2 bg-slate-900 text-white font-bold py-3 rounded-2xl">
        <Download size={16} /> Download PDF Invoice
      </button>
      <button onClick={onEditInvoice} className="w-full flex items-center justify-center gap-2 bg-amber-500 text-white font-bold py-3 rounded-2xl">
        <Edit3 size={16} /> Edit Invoice
      </button>
      <button onClick={onNewInvoice} className="w-full flex items-center justify-center gap-2 bg-white border border-slate-300 text-slate-700 font-semibold py-3 rounded-2xl">
        <PlusCircle size={16} /> Make Another Invoice
      </button>
      <Link to="/admin-dashboard/billing" onClick={onClose} className="w-full flex items-center justify-center gap-2 bg-emerald-50 text-emerald-700 font-semibold py-3 rounded-2xl">
