...
const ClientDashboard = () => {
    ...
    return (
        <div className="px-4 py-5 space-y-6 max-w-2xl mx-auto">
            ...
            {/* ✨ Recent Activity List - CARDS ONLY */}
            {isApproved && recentActivity.length > 0 && (
                <div>
                    <div className="flex items-center justify-between mb-3 mt-6">
                        <h2 className="text-slate-800 font-bold text-xl">Recent Activity</h2>
                    ...
                    <div className="space-y-3">
                        {recentActivity.map((record) => (
                            <TrackingCard 
                                key={record._id} 
                                record={record} 
                                onClick={() => handleRecordClick(record.raw)} 
                                forceShowDesktop={true} // ✨ THIS FORCES IT TO SHOW ON DESKTOP!
                            />
                        ))}
                    </div>
                </div>
