// customers/components/SearchBar.jsx
import { Search, SlidersHorizontal } from 'lucide-react';

export const SearchBar = ({ search, onSearchChange, onFilterOpen, activeFilterCount }) => (
  <div className="flex gap-2">
    <div className="flex-1 flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-4 py-3">
      ...
      <input
        ...
        placeholder="Name, ID, city, line…"
        className="flex-1 text-base text-slate-700 placeholder-slate-400 bg-transparent outline-none"
      />
    </div>
    <button
      onClick={onFilterOpen}
      className={`relative flex items-center justify-center px-4 rounded-xl border text-base font-medium transition-colors
        ${activeFilterCount > 0
