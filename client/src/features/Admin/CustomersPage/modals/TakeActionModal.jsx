...
import { useState, useEffect } from 'react';
import { X, FileQuestion, CheckCircle2, XCircle, Loader2, Clock, Plus, Trash2 } from 'lucide-react';
import { toast } from 'sonner';
import { api } from '../../../../services/api';
...
export const TakeActionModal = ({ client, onClose, onApprove, onReject }) => {
  ...
  // ✨ NEW: State to hold multiple requests
  const [drafts, setDrafts] = useState([{ documentType: 'gstCert', note: '' }]);
  ...
  const handleSendRequests = async () => {
    // Validate all drafts
    const invalid = drafts.some(d => !d.note.trim());
    if (invalid) {
      toast.error('Please provide a message for all requested documents.');
      return;
    }

    setSending(true);
    try {
      // Send the array to the upgraded backend
      await api.createDocumentRequest(client._id, { requests: drafts });
      toast.success('Requests sent to client');
      onClose();
    } catch (err) {
  ...
  const addDraft = () => setDrafts([...drafts, { documentType: 'dlCert', note: '' }]);
  const removeDraft = (index) => setDrafts(drafts.filter((_, i) => i !== index));
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[70] bg-white rounded-t-2xl px-5 pt-4 pb-6 max-h-[85dvh] overflow-y-auto">
        ...
        {activeRequests && activeRequests.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4 space-y-2">
            <p className="text-xs font-bold text-amber-700 uppercase tracking-wide flex items-center gap-1.5">
              <Clock size={12} /> Waiting on {activeRequests.length} document{activeRequests.length > 1 ? 's' : ''}
            </p>
            {activeRequests.map((r) => (
              <div key={r._id} className="border-t border-amber-200/50 pt-2 mt-2 first:border-0 first:pt-0 first:mt-0">
                <span className="text-xs font-bold bg-amber-200/50 text-amber-800 px-2 py-0.5 rounded mr-2">
        ...
        {view === 'choose' ? (
          <div className="space-y-2.5">
            <button
              ...
              className="w-full flex items-center gap-3 bg-blue-50 text-blue-700 font-semibold text-base px-4 py-3.5 rounded-xl hover:bg-blue-100"
            >
              <FileQuestion size={20} /> Ask for document updates
            </button>
            <button
              onClick={() => { onApprove(client); onClose(); }}
              className="w-full flex items-center gap-3 bg-emerald-500 text-white font-bold text-base px-4 py-3.5 rounded-xl hover:bg-emerald-600"
            >
              <CheckCircle2 size={20} /> Approve Account
            </button>
            <button
              onClick={() => { onReject(client); onClose(); }}
              className="w-full flex items-center gap-3 bg-red-50 text-red-600 font-semibold text-base px-4 py-3.5 rounded-xl hover:bg-red-100"
            >
              <XCircle size={20} /> Reject Account
            </button>
          ...
          <div className="space-y-4">
            {/* ✨ NEW: Render multiple request drafts */}
            {drafts.map((draft, index) => (
              <div key={index} className="bg-slate-50 border border-slate-200 p-3 rounded-xl relative">
                {drafts.length > 1 && (
                  <button onClick={() => removeDraft(index)} className="absolute top-3 right-3 text-red-400 hover:text-red-600">
                    <Trash2 size={16} />
                  </button>
                )}
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wide block mb-1">Document {index + 1}</label>
                <select
                  value={draft.documentType}
                  onChange={(e) => updateDraft(index, 'documentType', e.target.value)}
                  className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm mb-3 focus:border-blue-400 outline-none"
                >
                  <option value="gstCert">GST Certificate</option>
                  <option value="dlCert">Drug License (20B / 21B)</option>
                  <option value="panCard">PAN Card</option>
                ...
                <textarea
                  ...
                  placeholder="Reason for request (e.g., Image is blurry)"
                  rows={2}
                  className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm focus:border-blue-400 outline-none resize-none"
                />
              </div>
            ))}
            ...
            <button 
              onClick={addDraft}
              className="flex items-center gap-1.5 text-sm font-semibold text-blue-600 hover:text-blue-800 px-1"
            >
              <Plus size={16} /> Add another document
            </button>

            <div className="flex gap-3 pt-2">
              <button onClick={() => setView('choose')} className="flex-1 bg-slate-100 text-slate-700 font-semibold py-3 rounded-xl">Back</button>
              <button
                onClick={handleSendRequests}
                disabled={sending}
                className="flex-1 bg-blue-600 text-white font-bold py-3 rounded-xl disabled:opacity-50 flex items-center justify-center gap-1.5"
              >
                {sending ? <Loader2 size={16} className="animate-spin" /> : null} Send Requests
              </button>
            </div>
          </div>
        )}
