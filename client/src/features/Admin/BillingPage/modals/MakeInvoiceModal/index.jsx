...
import { downloadInvoicePDF, printInvoicePDF } from '../../pdf/invoice/generateInvoicePdf';
...
export const MakeInvoiceModal = ({ onClose }) => {
  ...
  const fetchMasterData = async () => {
    ...
    try {
      ...
      toast.error('Failed to load fresh data', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const invoiceData = {
        ...
        // ★ FIXED: Safely fallback to the selected client's current balance if the backend doesn't return it
  ...
  const getMergedItemsForPDF = () => {
    return items.map((localItem, index) => {
      // Safely grab the backend version of this exact item
      const backendItem = generatedInvoice.products[index];
      return {
        ...localItem,
        // Inject the short code from the backend!
  ...
  const handleDownloadPDF = () => {
    ...
    downloadInvoicePDF(
      generatedInvoice,
      getMergedItemsForPDF(), // ★ FIXED: Now passing merged data!
      totalTaxable,
      totalCGST,
      totalSGST,
      netAmount,
    ...
    <>
      <div className="fixed inset-0 z-50 bg-black/60 flex items-end">
        <div className="w-full bg-white rounded-t-2xl flex flex-col overflow-hidden" style={{ height: '80dvh' }}>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4 transition-all duration-300 ease-out animate-fadeIn">
            {step === 1 && (
              <Step1
