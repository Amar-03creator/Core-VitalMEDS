        ...
        <div className="w-full md:max-w-xl bg-white rounded-t-2xl md:rounded-2xl max-h-[90dvh] overflow-hidden flex flex-col">
          ...
          <div className="bg-white px-5 pt-4 pb-3 border-b border-slate-100 flex justify-between items-start shrink-0">
            <div>
              ...
              <div className="flex items-center gap-2 mt-1.5">
                ...
                <span className="text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md text-sm font-black border border-emerald-200 shadow-sm">
          ...
          <div className="px-4 md:px-5 py-4 space-y-4 overflow-y-auto flex-1 min-h-0 bg-slate-50/50">
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                <div className="flex flex-wrap gap-1.5 items-center">
                  <span className={`px-2 py-0.5 rounded text-xs sm:text-sm font-black uppercase tracking-wider border shadow-sm ${order.billPreference === 'Cash' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>
                    Req: {order.billPreference || 'Credit'}
                  </span>
                  {order.invoiceBillType && order.invoiceBillType !== order.billPreference && (
                    <span className={`px-2 py-0.5 rounded text-xs sm:text-sm font-black uppercase tracking-wider border shadow-sm ${order.invoiceBillType === 'Cash' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>
            ...
            {order.pricingSharedAt && (
              <p className="text-sm sm:text-sm text-slate-500 flex items-center gap-1.5 font-medium"><CheckCircle2 size={14} className="text-emerald-500" /> Pricing shared with client on {formatDateTime(order.pricingSharedAt)}</p>
            )}
