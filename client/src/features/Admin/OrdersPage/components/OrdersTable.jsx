  ...
  return (
    <div className="hidden md:block bg-white rounded-2xl border border-slate-200 overflow-hidden">
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-slate-50 text-slate-500 text-base font-bold uppercase text-left tracking-wide">
            <th className="px-4 py-2">Order ID</th>
            <th className="px-4 py-2">Date & Time</th>
            <th className="px-4 py-2">Customer</th>
            <th className="px-4 py-2">City / Route</th>
          ...
          {orders.map((order) => {
            ...
            return (
              <tr key={order._id} onClick={() => onOpen(order)} className="border-t border-slate-100 hover:bg-slate-50 cursor-pointer">
                <td className="px-4 py-2 font-mono text-sm text-slate-500">{order.orderId}</td>
                <td className="px-4 py-2 text-slate-500 text-sm whitespace-nowrap">{formatDateTime(order.createdAt)}</td>
                ...
                <td className="px-4 py-2 text-slate-500">{order.clientId?.city || order.clientId?.deliveryRoute || '—'}</td>
                ...
                <td className="px-4 py-2">
                  <span className="inline-flex items-center gap-1 text-slate-500 text-sm"><SourceIcon size={12} /> {source.label}</span>
                </td>
                <td className="px-4 py-2">
                  <span className={`inline-flex items-center gap-1 text-sm font-bold px-2 py-0.5 rounded-full ${meta.bg} ${meta.color}`}>
