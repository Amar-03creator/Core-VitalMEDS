...
export const TopParties = ({ data }) => {
  const [partySort, setPartySort] = useState('volume');
  const [partySortOpen, setPartySortOpen] = useState(false);

  const parties = data[partySort] || [];
  const partySortLabels = { volume: 'Sort: Volume', speed: 'Sort: Pay Speed', mvp: 'Sort: MVP' };
        ...
        <div className="relative">
          ...
          {partySortOpen && (
            <div className="absolute right-0 top-8 z-10 bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden w-40">
              {[{ key: 'volume', label: '📦 Volume' }, { key: 'speed', label: '⚡ Pay Speed' }, { key: 'mvp', label: '⭐ MVP Score' }].map(({ key, label }) => (
                <button key={key} onClick={() => { setPartySort(key); setPartySortOpen(false); }} className={`w-full text-left px-4 py-2.5 text-sm font-medium transition-colors ${partySort === key ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-50'}`}>
                  {label}
                </button>
              ))}
            </div>
