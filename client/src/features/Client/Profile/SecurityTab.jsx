...
const SecurityTab = ({ profile }) => {
  ...
  const originalEmail = profile?.contacts?.find(c => c.isPrimary)?.email || profile?.contacts?.[0]?.email || '';
  const originalPhone = profile?.contacts?.find(c => c.isPrimary)?.phone || profile?.contacts?.[0]?.phone || '';
  ...
  const handleContactInitiate = async (e) => {
    ...
    try {
      ...
      // 1. Pre-check MongoDB to ensure email isn't already taken
      if (emailChanged) {
        await authAxios.post('/api/clients/me/contact/precheck', { email: contactForm.email });
      }

      // 2. Tell AWS Cognito to update (This auto-sends the OTP to the new email)
      ...
      if (emailChanged) {
        ...
        // If only the phone changed, just save directly to MongoDB
        await authAxios.put('/api/clients/me/contact', contactForm);
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      // 1. Verify the code with AWS Cognito
      await verifyContactOtp('email', otpCode);

      // 2. ONLY if verification succeeds, save the new email to MongoDB
      await authAxios.put('/api/clients/me/contact', contactForm);
      ...
      // Force logout so they sign back in with their new email
  ...
  return (
    <div className="space-y-6 relative">
      <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex items-start gap-3 shadow-sm">
        ...
        <div>
          <h3 className="text-emerald-900 font-bold text-base">Account Security</h3>
          <p className="text-emerald-800 text-sm mt-1 leading-relaxed">
            Your credentials are secured by AWS. If you change your email, you will need to verify it with a 6-digit code.
          </p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center">
              <h3 className="font-bold text-xl text-slate-900">Verify New Email</h3>
