...
// ✨ Added 'forceShowDesktop' prop with a default of false
export default function TrackingCard({ record, onClick, forceShowDesktop = false }) {
  return (
    <button 
      onClick={onClick} 
      // ✨ Conditionally apply 'md:hidden' based on the prop
      className={`w-full text-left bg-white rounded-xl border border-slate-200 p-4 shadow-sm mb-2 relative hover:bg-slate-50 transition-colors ${forceShowDesktop ? '' : 'md:hidden'}`}
    >
      {record.showEyeIcon && (
        <div className="absolute top-3 right-3 text-violet-600 bg-violet-50 p-1.5 rounded-full z-10 pointer-events-none">
          <Eye size={18} />
