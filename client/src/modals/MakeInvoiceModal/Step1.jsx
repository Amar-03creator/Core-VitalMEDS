  ...
  <div className="flex flex-col h-full pb-2">
    <div className="flex-1 flex flex-col space-y-5">
      ...
      {lockClient && selectedClient ? (
        <div>
          <label className="text-base font-semibold text-slate-700 block mb-2">Party / Client</label>
          <input
            value={selectedClient.establishmentName}
            disabled
            className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-1.5 text-lg font-mono font-bold text-slate-600 outline-none cursor-not-allowed"
          />
        </div>
      ) : (
        /* Editable search – unchanged */
