...
//                   className={`shrink-0 px-2.5 py-1 rounded-full text-sm font-medium whitespace-nowrap transition-all duration-200
//                     ${i === currentIndex ? 'bg-slate-800 text-white scale-100' : item.clientEdited ? 'bg-amber-100 text-amber-800 border border-amber-300 shadow-[0_0_8px_rgba(251,191,36,0.3)]' : 'bg-slate-100 text-slate-600 hover:bg-slate-300'}
//                     ${addingHighlight && i === currentIndex ? 'animate-pulse bg-emerald-100 text-emerald-700' : ''}`}>
...
//       <div className="flex gap-2 pt-3 mt-2 border-t border-slate-100 shrink-0 bg-white z-10">
//         <button onClick={onBack} className="flex-1 bg-slate-100 py-2.5 rounded-lg text-md font-bold hover:bg-slate-200 inline-flex items-center justify-center gap-1 transition-colors">
...
//           className="flex-[2] bg-slate-800 disabled:bg-slate-300 text-white font-bold py-2.5 rounded-lg text-md hover:bg-slate-700 inline-flex items-center justify-center gap-1 transition-colors"
//         >
...
export const Step2 = ({
  ...
  const handleTouchStart = (e) => {
    // ✨ FIX 1: Removed the premature blur() that was collapsing the keyboard!
    touchStartX.current = e.touches[0].clientX;
    touchStartY.current = e.touches[0].clientY;
    touchMoved.current = false;
  };
  ...
  const handleTouchEnd = () => {
    ...
    const dy = touchStartY.current - touchEndY.current;
    if (Math.abs(dx) > 60 && Math.abs(dx) > Math.abs(dy)) {
      document.activeElement?.blur(); // ✨ Only blur if they actually swiped the card!
      if (dx > 0) goNext();
      else goPrev();
    }
    touchMoved.current = false;
  ...
  return (
    // ✨ FIX 2: Replaced 'h-full overflow-hidden' with natural flow layout so hitboxes stay glued to inputs
    <div className="w-full flex flex-col">
      <div ref={searchRef} className="relative mb-4 z-40 shrink-0">
