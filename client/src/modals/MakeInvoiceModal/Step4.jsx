// // src/features/Admin/BillingPage/modals/MakeInvoiceModal/Step4.jsx
// import { CheckCircle2, Download, Printer, PlusCircle, FileText, Edit3 } from 'lucide-react'; 
...
//           <div className="flex justify-between"><span className="text-slate-500">Order:</span><span className="font-mono font-bold">{generatedInvoice.orderId}</span></div>
//         )}
...
export const Step4 = ({ generatedInvoice, isPrefilled, onDownloadPDF, onPrintPDF, onNewInvoice, onEditInvoice, onClose }) => (
  <div className="space-y-5">
    ...
    <div className="flex flex-col gap-3">
      ...
      {isPrefilled ? (
        <button onClick={onClose} className="w-full bg-emerald-600 text-white font-bold py-3.5 rounded-xl hover:bg-emerald-700 transition-colors shadow-md">
          Start Packing
        </button>
      ) : (
        <>
          ...
          <Link to="/admin-dashboard/billing" onClick={onClose} className="w-full flex items-center justify-center gap-2 bg-emerald-50 text-emerald-700 font-semibold py-3.5 rounded-xl hover:bg-emerald-100 transition-colors">
            <FileText size={16} /> See All Invoices
          </Link>
        </>
      )}
