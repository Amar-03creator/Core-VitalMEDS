...
exports.createClient = async (req, res) => {
  try {
    ...
    // ✨ 4. SELF-HEALING ID GENERATOR LOOP
    let newClient;
    let retries = 0;
    const MAX_RETRIES = 5;
    ...
    while (retries < MAX_RETRIES) {
        try {
            // Get next ID
            const counter = await Counter.findByIdAndUpdate('client_seq', { $inc: { seq: 1 } }, { new: true, upsert: true });
            const clientId = counter.seq.toString(36).toUpperCase().padStart(3, '0');
            ...
            // Check if error is specifically E11000 duplicate key on `clientId`
            if (err.code === 11000 && err.keyPattern && err.keyPattern.clientId) {
                console.warn(`[Auto-Heal] clientId duplicate detected. Retrying... (${retries + 1}/${MAX_RETRIES})`);
                retries++;
                continue; // Database desync found. Loop restarts to grab the next counter value.
            }
            // If it's a different error (like validation failure), throw it normally
            throw err; 
        }
    }

    if (retries === MAX_RETRIES) {
        throw new Error('Failed to generate a unique Client ID after multiple attempts. Please contact support.');
    }
...
exports.approveClient = async (req, res) => {
  try {
    ...
    if (!client) return res.status(404).json({ message: 'Client not found' });
    if (client.status !== 'Pending') return res.status(400).json({ message: 'Only pending clients can be approved' });
