// server/src/helpers/invoicePDF/generateInvoicePdf.js

const { jsPDF } = require('jspdf');
const { CONTENT, CONTENT_PAD } = require('./pdfGeometry');
const { formatPacking, formatExpiryForBill, getPageSizes } = require('./pdfHelpers');
const { drawPageBorder, drawHeader, drawItemsTable, drawBillTotalLine, drawFooter } = require('./pdfDraw');
...
const generateInvoicePdfDoc = (
  ...
  // ── LIVE PROFILE FALLBACKS ──
  const addr = adminProfile.address || {};
  const addressParts = [addr.street, addr.city].filter(Boolean).join(', ');
  const dynamicAddress = addressParts ? `${addressParts.toUpperCase()}${addr.pincode ? `-${addr.pincode}` : ''}` : '';

  const livePhones = [
    adminProfile.proprietor?.phones?.[0],
    adminProfile.competentPerson?.phones?.[0]
  ].filter(Boolean).join(', ');

  const liveDlString = (adminProfile.drugLicenses || [])
    .map(dl => dl.dlNumber)
    .filter(Boolean)
    .join(', ');

  // ✨ THE TIME-TRAVEL FIX: Prioritize the Frozen Snapshot!
  const snap = invoiceData.sellerSnapshot;

  const seller = {
    name: snap?.establishmentName?.toUpperCase() || adminProfile.establishmentName?.toUpperCase() || 'M/S MILA AGENCIES',
    address: snap?.addressString || dynamicAddress || 'UPENDRA BHANJA COLONY, BERHAMPUR-760008',
    dist: snap?.district?.toUpperCase() || addr.district?.toUpperCase() || 'GANJAM',
    phone: (snap?.proprietorPhone || snap?.cpPhone) 
            ? [snap.proprietorPhone, snap.cpPhone].filter(Boolean).join(', ') 
            : livePhones || '9437176172, 9438537878',
    dlNo: snap?.drugLicenses || liveDlString || 'SL-GA-I-3902-W, GA-I-3903-WC, GA-I-3904-WX',
    gstin: snap?.gstin || adminProfile.gstinAdmin || '21CIJPM8416K1Z2',
    drugsBazaarId: snap?.drugsBazaarId || adminProfile.drugsBazaarId || 'MILBER008548',
  };
...
module.exports = { generateInvoicePdfDoc };