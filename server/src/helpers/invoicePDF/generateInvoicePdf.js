...
const { CONTENT } = require('./pdfGeometry.js');
const { formatPacking, formatExpiryForBill, getPageSizes } = require('./pdfHelpers.js');
const { drawPageBorder, drawHeader, drawItemsTable, drawBillTotalLine, drawFooter } = require('./pdfDraw.js');
...
exports.generateInvoicePdfBuffer = async (invoiceData) => {
    ...
    // Since the backend explicitly saves all calculated values, we just map them for display
    const enhancedItems = allItems.map(item => {
        ...
        return { ...item.toObject(), discountDisplay };
    });
    ...
    const pages = pageSizes.map((size, idx) => {
        ...
        const itemRows = pageItems.map((item, rowIdx) => {
            ...
            if (item.taxableValue > 0) {
                if (isInterstate) gstRate = (item.igst / item.taxableValue) * 100;
                else gstRate = ((item.cgst + item.sgst) / item.taxableValue) * 100;
            }
    ...
    const taxSummaryStr = Object.values(
        enhancedItems.reduce((acc, item) => {
            ...
            if (item.taxableValue > 0) {
                rate = isInterstate 
                    ? Math.round((item.igst / item.taxableValue) * 100) 
                    : Math.round(((item.cgst + item.sgst) / item.taxableValue) * 100);
            }
            const key = `${item.hsn || '0'}|${rate}`;
            if (!acc[key]) acc[key] = { hsn: item.hsn || '', rate, qty: 0, taxable: 0, cgst: 0, sgst: 0, igst: 0 };
            
            acc[key].qty += item.billedQty;
