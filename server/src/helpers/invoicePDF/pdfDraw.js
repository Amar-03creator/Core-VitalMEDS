// server/src/helpers/pdfDraw.js

const autoTableModule = require('jspdf-autotable');
const autoTable = autoTableModule.default || autoTableModule;const { A4, BORDER, CONTENT, BILL_TOTAL_GAP_MM } = require('./pdfGeometry.js');
const { numberToWords, formatDateIndian } = require('./pdfHelpers.js');

exports.drawPageBorder = function(doc) {
  doc.setDrawColor(0, 0, 0);
  doc.setLineWidth(0.18);
  doc.rect(BORDER.x, BORDER.y, BORDER.w, BORDER.h);
};

exports.drawHeader = function(doc, info) {
...
exports.drawItemsTable = function(doc, page, head, COLS, startY, showPageTotal, isInterstate) {
  const body = [...page.rows];

  if (showPageTotal) {
    const row = [
      { content: `Page total (${page.pageTotalItems})`, colSpan: 10, styles: { halign: 'left', fontStyle: 'bold' } },
  ...
  const columnStyles = isInterstate ? {
    ...
    4: { cellWidth: COLS.pack, halign: 'left' },
    5: { cellWidth: COLS.batch, halign: 'left' },
    6: { cellWidth: COLS.exp, halign: 'center' },
    7: { cellWidth: COLS.mrp, halign: 'right' },
    8: { cellWidth: COLS.rate, halign: 'right' },
    9: { cellWidth: COLS.qty, halign: 'right' },
    10: { cellWidth: COLS.disc, halign: 'right' },
    11: { cellWidth: COLS.taxable, halign: 'right' }, 
    12: { cellWidth: COLS.igst, halign: 'right' },
    ...
    7: { cellWidth: COLS.mrp, halign: 'right' },
    8: { cellWidth: COLS.rate, halign: 'right' },
    9: { cellWidth: COLS.qty, halign: 'left' },
    10: { cellWidth: COLS.disc, halign: 'right' },
    11: { cellWidth: COLS.taxable, halign: 'right' },
    12: { cellWidth: COLS.cgst, halign: 'right' },
    13: { cellWidth: COLS.sgst, halign: 'right' },
    14: { cellWidth: COLS.total, halign: 'right' },
  };

  // Note: calling autoTable as a function, which is the Node.js standard
  autoTable(doc, {
    startY,
    ...
    headStyles: { fillColor: [241, 245, 249], textColor: [0, 0, 0], fontStyle: 'bold', halign: 'center', fontSize: 7 },
...
exports.drawBillTotalLine = function(doc, tableEndY, totals) {
  const {
...
exports.drawFooter = function(doc, startY, info) {
  ...
  doc.setFontSize(6.5);
  doc.setFont('helvetica', 'bold');
  const totalUnits = allItems.reduce((s, i) => s + (i.billedQty || i.chargeableQty + (i.freeQty || 0)), 0); 
